...
function StoryBubble({
  src, username, size = 58, onClick, delay = 0, hasRing = true,
}: {
  src?: string; username?: string; size?: number;
  onClick?: () => void; delay?: number; hasRing?: boolean;
}) {
  return (
    <Box
      onClick={onClick}
    ...
    >
      {hasRing ? (
        <div className="story-ring" style={{ width: size + 4, height: size + 4 }}>
          <div className="story-ring-inner" style={{ width: size, height: size }}>
            <img
              src={src || BlankProfileImage} alt={username}
      ...
      {username && (
        <Typography sx={{
          fontSize: "0.66rem", fontFamily: "'Inter', sans-serif", fontWeight: 400,
          color: (t) => t.palette.text.secondary,
          maxWidth: size + 8, whiteSpace: "nowrap", overflow: "hidden",
...
const HomePage = () => {
  ...
  const fetchPosts = async () => {
    try {
      if (currentUser?.id) { const res = await getPosts(); setPosts(res.data); }
  ...
  const fetchStories = async () => {
    try {
      ...
      const group = (arr: any[]) => Object.values(
    ...
    >
      ...
      >
        ...
        >
          <Box
            sx={{
              display: "flex", gap: "16px", overflowX: "auto", pb: "2px",
            ...
            {fetchingStories
              ...
              : followingStories.map((us, idx) => (
                  <StoryBubble
                    ...
                    delay={idx * 40}
                    onClick={() => { setSelectedStoryIndex(selfStories.length + idx); setOpenStoryDialog(true); }}
                  />
                ))}
          </Box>
        </Box>
          ...
          {loadingPosts ? (
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: "10px", p: "14px" }}>
                    ...
                    <Box flex={1}>
                      ...
                      <Skeleton width="18%" height={9} sx={{ bgcolor: (t) => t.palette.action.hover, borderRadius: "6px", mt: "5px" }} />
                    </Box>
                  </Box>
                  <Skeleton variant="rectangular" height={260} sx={{ bgcolor: (t) => t.palette.action.hover }} />
                  <Box sx={{ display: "flex", gap: "14px", p: "12px 14px" }}>
                    {[52, 44, 36].map((w, j) => (
                      <Skeleton key={j} width={w} height={10} sx={{ bgcolor: (t) => t.palette.action.hover, borderRadius: "5px" }} />
                    ))}
                  </Box>
                </Box>
              ))}
            ...
            <Box
              sx={{
                ...
                minHeight: "50vh", px: 4, textAlign: "center",
            ...
            >
              ...
              <Typography sx={{
                ...
                fontSize: "0.95rem", color: (t) => t.palette.text.primary, mb: 0.75,
              }}>
                Nothing here yet
              </Typography>
              <Typography sx={{
                fontSize: "0.82rem", color: (t) => t.palette.text.disabled,
              ...
              <Box
                ...
                sx={{
                  mt: 2.5, px: 3, py: 1, borderRadius: "10px",
